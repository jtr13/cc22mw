...
![After adding formatting into the Iris file shown above, the Excel takes up much more space than the csv file](resources/fig1.png)
...
![Excel worksheets can be very cumbersome and easy to break; hug your business school friends](resources/fig2.png)
#### Ingesting Excel files into R
You can call the `read_excel()` function from `readxl` library to import the file into R.

```{r}
df <- read_csv('resources/iris.xlsx')
glimpse(df)
```
### JSON and XML
...
![More questions are asked about JSON than any other data exchange format. I do believe that the low percengage of .csv related questions speak less to its lack of popularity, more to its simplicity](sample_files/fig2.png)

#### Ingesting JSON and XML files into R
You can call the `fromJSON()` function from `rjson` library to import the file into R.

```{r}
data <- fromJSON(file = 'resources/sample.json')
print(data)
```
```{r}
data_xml <- xmlParse(file = "resources/sample.xml")
print(data_xml)
...
```{r}
dataframe <- xmlToDataFrame("resources/sample.xml")
print(dataframe)
```
